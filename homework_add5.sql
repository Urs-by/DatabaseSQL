...
SELECT us.id, m.body
FROM users us 
JOIN messages m ON us.id = m.from_user_id AND us.id=1
UNION
SELECT us.id, m.body
FROM users us 
JOIN messages m ON us.id = m.to_user_id AND us.id=1;

SELECT * FROM message;

-- Получите список медиафайлов пользователя с количеством лайков(media m, likes l ,users u)

SELECT us.id, m.filename, COUNT(l.media_id)  AS count_likes
FROM users us
...
-- или для проверки
SELECT us.id, m.filename, COUNT(l.media_id) OVER (PARTITION BY l.media_id) AS count_likes
FROM users us
JOIN media m ON us.id = m.user_id
JOIN likes l ON m.id = l.media_id
ORDER BY us.id;
...
SELECT   us.id, COUNT(uc.community_id) AS 'count_group'
FROM users us
JOIN users_communities uc ON us.id = uc.user_id
GROUP BY us.id;
