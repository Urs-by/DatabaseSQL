...
ALTER TABLE users ADD FOREIGN KEY(id) REFERENCES clients(id);
ALTER TABLE posts ADD FOREIGN KEY(user_id) REFERENCES clients(id);
ALTER TABLE posts ADD COLUMN login VARCHAR(255);
UPDATE posts SET login = (SELECT clients.login FROM clients WHERE clients.id=posts.user_id);
SELECT title, full_text, user_id, login FROM posts;
SELECT * FROM posts WHERE login = 'Mikle';
