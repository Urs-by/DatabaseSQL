...
SELECT us.id, us.firstname AS "имя", us.lastname AS "фамилия", prof.hometown AS "город", media.filename AS "аватарка"
FROM users us
JOIN `profiles` prof ON us.id = prof.user_id
LEFT JOIN media ON prof.photo_id = media.id;

SELECT id, firstname AS "имя", lastname AS "фамилия",
(SELECT hometown FROM `profiles` WHERE user_id = users.id) AS "город",
(SELECT filename FROM media WHERE id = (SELECT photo_id FROM `profiles` WHERE user_id = users.id))AS "аватарка"
FROM users;

-- выбрать фотографии (filename) пользователя с email: arlo50@example.org . ID типа медиа, соответствующий фотографиям неизвестен.
SELECT filename AS "аватарка" , us.email AS "почта"
FROM media
JOIN users us
ON us.id = media.user_id 
AND us.email = "arlo50@example.org" 
AND (filename LIKE '%jpg' OR filename LIKE '%bmp' OR filename LIKE '%png');
-- WHERE us.email = "arlo50@example.org" and (filename LIKE '%jpg' OR filename LIKE '%bmp' OR filename LIKE '%png');



SELECT filename AS "аватарка"  FROM  media
WHERE  user_id IN (SELECT id from users WHERE email = "arlo50@example.org" )  and
(filename LIKE '%jpg' OR filename LIKE '%bmp' OR filename LIKE '%png');

-- выбрать id друзей пользователя с id = 1

-- Из таблицы запросы в друзья выберем id тех, кто подтвердил статус 
SELECT target_user_id AS "ID друзей"
FROM friend_requests fr 
JOIN users us
ON us.id = fr.initiator_user_id
WHERE us.id=1 AND `status` = 'approved';

-- Здесь объединим 2 запроса чтобы добавить также кто напросился в друзья
SELECT us.id AS  "ID друзей"
FROM users us 
JOIN friend_requests fr ON us.id = fr.target_user_id AND fr.status = 'approved' AND fr.initiator_user_id = 1 
UNION 
SELECT us.id
FROM users us 
JOIN friend_requests fr ON us.id = fr.initiator_user_id AND fr.status = 'approved' AND fr.target_user_id = 1;

-- Список медиафайлов пользователей, указав название типа медиа (id, filename, name_type)

SELECT us.id, concat(us.firstname ,' ', us.lastname) AS "ФИо", filename AS "медиафайлы" , mt.name_type AS "тип файлов"
FROM users us
JOIN media m ON us.id = m.user_id 
